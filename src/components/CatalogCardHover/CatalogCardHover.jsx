...
const CatalogCardHover = ({ index, animeData, id }) => {
	return (
		<div
			className={`${styles.ccHoverWrapper} ${
				index % 5 === 0 ? styles.L : styles.R
			}`}
		>
			<div className={styles.ccHoverHeader}>
				<span className={styles.ccHoverTitle}>{animeData?.titles?.ru}</span>
				<span className={styles.ccHoverEngTitle}>
					{animeData?.titles?.title || animeData?.titles?.eng}
				</span>
			</div>
			<div className={styles.ccHoverInfo}>
				...
				<div className={styles.infoItem}>
					...
					<span className={styles.infoItemC}>
						...
						{animeData?.episodes_count?.total} [{animeData?.time?.formated}]
					</span>
				</div>
			</div>
			<div className={styles.ccDescWrapper}>
				<div className={styles.ccHoverDesc}>
					<span>{animeData?.description}</span>
				</div>
				<Link to={`/anime/${id}`} className={styles.ccDescMore}>
					Подробнее...
				</Link>
			</div>
